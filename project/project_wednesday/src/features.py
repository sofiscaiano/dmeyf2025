...
def feature_engineering_min_max_avg(df: pl.DataFrame, columnas: list[str], window: int, f_min: True, f_max: True, f_avg: True) -> pl.DataFrame:
    """
...
def create_embedding_rf(df: pl.DataFrame) -> pl.DataFrame:
    ...
    X_train, y_train, X_test, y_test, w_train, feature_name  = train_test_split(df=df, undersampling=False, mes_train=[202101, 202102, 202103], mes_test=[202104])
...
def create_embedding_lgbm_rf(df: pl.DataFrame):
    ...
    X_train, y_train, X_test, y_test, w_train, feature_name = train_test_split(
        df=df,
        undersampling=False,
